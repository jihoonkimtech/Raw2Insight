...
- 지원 센서 프로토콜:
  ...
  - `digital`
  - `i2c`
  - `dht` (DHT 계열 단선 센서, DIN01/DIN02 단자)
- 지원 액추에이터 타입:
  - `digital_out`
  - `pwm`
...
- 레지스터 기반 센서는 선택 속성인 `trigger_sequence`, `trigger_delay`로 읽기 직전마다 측정 명령을 보낼 수 있습니다.
- 단선(DHT) 센서는 `BaseDHTSensor`를 상속받습니다. 40비트 타이밍 디코딩은 MCU의 `read_dht_bytes`가 수행하고, 드라이버는 5바이트를 파싱합니다. `min_interval`로 최소 측정 간격을 지정합니다.
- 기본 제공 드라이버: `AHT20`(온도/습도), `MPU6050`(3축 가속도·자이로, 가속도 크기, Roll/Pitch, 온도), `DHT11`(온도/습도)
- DHT11은 3.3V 전원과 DATA–VCC 사이 4.7kΩ~10kΩ 풀업 사용을 권장합니다. MCU 로직은 3.3V입니다.
...
- 센서 루프는 약 5Hz(0.2초 주기)로 동작하며, 대시보드는 매 주기 최신 값 1개만 받아 차트를 갱신합니다.
- 새로 접속한 브라우저는 서버 메모리에 보관된 최근 60초 기록(`dashboard_snapshot`)을 받아 즉시 그립니다.
- 상단 요약(센서 수, 이상 감지, 작동 중인 출력, 갱신 속도, CPU, RAM)과 센서 카드 그리드로 구성됩니다.
- 노트북/모니터에서는 센서 수에 맞춰 카드 배치를 계산해 스크롤 없이 한 화면에 표시하며, 768px 이하에서는 세로 스크롤로 전환합니다.
- 다크 테마가 기본이며 상단 버튼으로 라이트 테마로 전환할 수 있습니다.
- 장치 등록/관리 화면과 모니터링 화면이 분리된 SPA 구조입니다.

### 💾 7. 설정 프리셋 (Preset)
- 장치 설정은 `/app/data/device_config.db`(SQLite)에 저장된다. 장치 설정 탭의 **설정 프리셋** 패널에서 이 설정을 JSON으로 옮길 수 있다.
  - **현재 설정 내보내기**: 등록된 센서·출력을 JSON 파일로 내려받는다.
  - **파일에서 불러오기** / **붙여넣기**: 내려받은 JSON으로 전체 설정을 교체한다. 브라우저가 내용을 직접 보내므로 저장소에 파일을 둘 필요가 없다.
  - **프리셋 적용**: `python/presets/` 안의 JSON을 적용한다. `empty.json`은 모든 장치를 비운다.
- 적용 전에 전체 내용을 검사하며, 오류가 있으면 기존 설정을 그대로 둔다.
- 출력 장치는 센서를 id가 아닌 **이름**(`linked_sensor`)으로 참조하므로 센서 이름은 중복될 수 없다.
- (선택) 로컬에 `python/presets/default.json`을 두면 DB가 비어 있을 때 실행 시 자동으로 불러온다. 이 폴더의 JSON은 `empty.json`을 제외하고 git에서 제외된다. 자동 적용을 끄려면 `RAW2INSIGHT_NO_SEED=1`을 설정한다.

### ⏱️ 8. 성능 관련 설정
- `python/main.py`의 `LOOP_PERIOD`로 루프 주기를 조정합니다(기본 0.2초).
- InfluxDB에는 센서별 1초 평균값을 기록하고, AI 입력은 메모리의 2초 평균(최대 10분)을 사용합니다. 재시작 시 DB에서 한 번 불러와 이어서 학습합니다.
- 주기마다 반복되는 디버그 로그는 기본으로 꺼져 있습니다. Python은 환경 변수 `RAW2INSIGHT_DEBUG=1`, MCU는 `sketch.ino`의 `MCU_VERBOSE`를 1로 설정하면 출력됩니다.
- AHT20은 측정 명령 후 루프를 멈추지 않고 다음 주기에 값을 읽으며, 2초 간격으로 측정합니다. DHT11은 1초 간격으로 측정합니다.
